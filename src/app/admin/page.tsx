...
import {
  ...
  Monitor, ExternalLink, Upload, ImageIcon, LocateFixed, GripVertical, LayoutGrid, LayoutList
} from "lucide-react";
import {
  DndContext,
  closestCenter,
...
function SortableSlideItem({ slide, onDelete }: { slide: ScreenSlide; onDelete: (s: ScreenSlide) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative rounded-xl overflow-hidden border border-border group">
      {/* drag handle */}
      <button
        {...attributes}
        {...listeners}
      ...
      <img src={slide.url} alt={slide.caption} className="w-full h-24 object-cover" />
      {slide.caption && (
        <p className="text-xs text-text-muted px-2 py-1 truncate bg-surface">{slide.caption}</p>
      )}
      <button
...
function SkrinMasjidEditor() {
  ...
  // Save helper — partial merge into screen_config
  const savePartial = async (partial: Partial<ScreenConfig>) => {
    const updated = { ...cfg, ...partial };
    setCfg(updated);
    await supabase.from("system_settings").update({ screen_config: updated }).eq("id", 1);
  };

  // Toggle a boolean "enabled" for a module
  ...
  // Upload mosque background photo
  const handlePhotoUpload = async (file: File) => {
    ...
    try {
      ...
      const { data: { publicUrl } } = supabase.storage.from("screen-assets").getPublicUrl(uploadData.path);
  ...
  const handleSlideUpload = async () => {
    ...
    try {
      ...
      if (error) { toast.error("Slide upload failed."); return; }
      const { data: { publicUrl } } = supabase.storage.from("screen-slides").getPublicUrl(uploadData.path);
      const { data: newSlide } = await supabase
        .from("screen_slides")
        .insert({ url: publicUrl, caption: slideCaption.trim(), sort_order: slides.length })
        .select()
        .single();
  ...
  // Drag-end: reorder locally + batch-save sort_order to DB
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    setSlides(reordered);
    // Persist new sort_order for every slide in one batch
    await Promise.all(
      reordered.map((s, i) =>
  ...
  // Auto-detect zone via geolocation
  const [zoneDetecting, setZoneDetecting] = useState(false);
  const detectZone = async () => {
    if (!navigator.geolocation) { toast.error("Geolocation tidak disokong."); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { toast.error("Akses lokasi ditolak."); setZoneDetecting(false); }
    );
  };

  if (loadingCfg) {
    return <div className="card p-6 flex items-center gap-2 text-text-muted text-sm"><Loader2 size={16} className="animate-spin" /> Loading screen settings…</div>;
  }
  ...
  return (
    <div className="card p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-2">
          <Monitor size={18} className="text-text" />
          <div>
      ...
      {/* Zone Selector */}
      <div>
        <label className="text-xs font-bold uppercase tracking-widest text-text-secondary mb-1 block">{t.skrinZone}</label>
        <div className="flex items-center gap-2">
          <input
            type="text"
            value={cfg.zone}
          ...
          >
            {zoneDetecting ? <Loader2 size={14} className="animate-spin" /> : <LocateFixed size={14} />}
            {zoneDetecting ? "…" : "Auto"}
          </button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border overflow-hidden divide-y divide-border">
        {MODULE_LIST.map(({ key, labelKey, descKey }) => {
          ...
          return (
            <div key={key} className="flex items-center justify-between p-4 bg-surface">
              <div>
                <p className="text-sm font-semibold text-text">{(t as Record<string, unknown>)[labelKey] as string}</p>
                <p className="text-xs text-text-muted mt-0.5">{(t as Record<string, unknown>)[descKey] as string}</p>
              </div>
              <button
                onClick={() => toggleModule(key)}
                className={`w-12 h-6 rounded-full flex items-center p-1 transition-colors ${enabled ? "bg-primary justify-end" : "bg-border justify-start"}`}
              >
      ...
      {cfg.gambar_masjid.enabled && (
        <div className="space-y-2">
          <label className="text-xs font-bold uppercase tracking-widest text-text-secondary block">{t.skrinUploadPhoto}</label>
          {cfg.gambar_masjid.url && (
            <img src={cfg.gambar_masjid.url} alt="Mosque" className="w-full h-36 object-cover rounded-xl border border-border" />
          )}
      ...
      {cfg.alert_iqamat.enabled && (
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-text-secondary mb-1 block">{t.skrinIqamatDelay}</label>
          <div className="flex items-center gap-3">
            <input
              type="number" min={1} max={60} value={iqamatDelay}
            ...
            />
            <button
              onClick={() => savePartial({ alert_iqamat: { ...cfg.alert_iqamat, delay_minutes: iqamatDelay } })}
          ...
          <div className="flex gap-2">
            {(["horizontal", "vertical"] as const).map(opt => {
              ...
              return (
                <button
                  key={opt}
                  onClick={() => savePartial({ panel_waktu: { ...cfg.panel_waktu, layout: opt } })}
                ...
                >
                  {opt === "horizontal" ? <LayoutGrid size={15} /> : <LayoutList size={15} />}
                  {opt === "horizontal" ? "Mendatar / Horizontal" : "Menegak / Vertical"}
                </button>
              );
            })}
          </div>
      ...
      {cfg.slideshow.enabled && (
        <div>
          <label className="text-xs font-bold uppercase tracking-widest text-text-secondary mb-1 block">{t.skrinSlideInterval}</label>
          <div className="flex items-center gap-3">
            <input
              type="number" min={3} max={120} value={slideInterval}
              onChange={e => setSlideInterval(Number(e.target.value))}
              className="w-24 border border-border rounded-xl px-3 py-2 text-sm outline-none focus:border-primary bg-background"
            />
            <button
              onClick={() => savePartial({ slideshow: { ...cfg.slideshow, interval_seconds: slideInterval } })}
              className="py-2 px-4 btn-primary text-sm"
            >
              {t.skrinSaveSettings}
            </button>
      ...
      {cfg.slideshow.enabled && (
        <div className="space-y-3">
          <p className="text-xs font-bold uppercase tracking-widest text-text-secondary">{t.skrinSlideshow}</p>

          {/* Existing slides — drag to reorder */}
          {slides.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-6 rounded-xl border border-dashed border-border">{t.skrinNoSlides}</p>
          ) : (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={slides.map(s => s.id)} strategy={rectSortingStrategy}>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {slides.map(slide => (
                    <SortableSlideItem key={slide.id} slide={slide} onDelete={handleDeleteSlide} />
                  ))}
                </div>
              </SortableContext>
            </DndContext>
          ...
          <div className="rounded-xl border border-dashed border-border p-4 space-y-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider">{t.skrinUploadSlide}</span>
            </div>
            <label className="flex items-center justify-center gap-2 py-2 border border-border rounded-xl text-sm text-text-muted hover:border-primary hover:text-primary transition cursor-pointer w-full">
              ...
              <input type="file" accept="image/*" className="hidden" onChange={e => setSlideFile(e.target.files?.[0] ?? null)} />
            </label>
            <input
              type="text" value={slideCaption} onChange={e => setSlideCaption(e.target.value)}
            ...
            >
              {slideUploading ? <Loader2 size={15} className="animate-spin" /> : <Upload size={15} />}
